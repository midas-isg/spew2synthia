...
def translate(fips):
    ...
    hh_ids, sc_ids, wp_ids = translate_pp(env_path, pp_csvs, code)
    translate_sc(env_path, sc_ids, code)
    translate_wp(env_path, wp_ids, code)
    cross_check_hids(fips, hh_ids)
    translate_hh(pp_csvs, code)


def translate_pp(env_path, pp_csvs, code):
    pp_csv = synth_file_name(code, 'people')
    gq_pp_csv = synth_file_name(code, 'gq_people')
    hh_ids, sc_ids, wp_ids = out_pp_file(env_path, pp_csvs, pp_csv, gq_pp_csv)
    save_pp_as_txt_with_reordering_columns(pp_csv, gq_pp_csv)
    return hh_ids, sc_ids, wp_ids


def translate_sc(env_path, sc_ids, code):
    sc_csv = out_file_name(code, 'schools')
    out_sc_file(env_path, sc_csv, sc_ids)
    save_sc_as_txt_with_reordering_columns(sc_csv)


def translate_wp(env_path, wp_ids, code):
    wp_csv = out_file_name(code, 'workplaces')
    out_wp_file(env_path, wp_csv, wp_ids)
    save_wp_as_txt_with_reordering_columns(wp_csv)


def cross_check_hids(fips, hh_ids):
    ref_hh_ids = out_ref_hh_file(find_csvs(conf.hh_prefix, fips))
...
def translate_hh(pp_csvs, code):
    ...
    out_hh_file(pp_csvs, hh_csv, gq_csv)
    save_hh_as_txt_with_reordering_columns(hh_csv, gq_csv)


def save_pp_as_txt_with_reordering_columns(pp_csv, gq_pp_csv):
    # csv columns (input):
...
def save_hh_as_txt_with_reordering_columns(hh_csv, gq_csv):
    # csv columns (input):
    #  0 RT,TYPE,SERIALNO,puma_id,HINCP,
    #  5 NP,place_id,SYNTHETIC_HID,longitude,latitude,
    # 10 RT,puma_id,ST,SEX,AGEP,
...
def save_sc_as_txt_with_reordering_columns(sc_csv):
    # csv columns (input):
...
def save_wp_as_txt_with_reordering_columns(wp_csv):
    # csv columns (input):
...
def out_pp_file(env_path, in_file_paths, pp_path, gq_pp_path):
    TYPE_COLUMN = 1
    HID_COLUMN = 7
    AGE_COLUMN = 14
    RELP_COLUMN = 17
...
def out_hh_file(in_file_paths, hh_path, gq_path):
    TYPE_COLUMN = 1
    RELP_COLUMN = 17
    columns = 29
    aid.mkdir(hh_path)
...
def test():
    ...
    prefix2csvs[conf.hh_prefix] = [path_us + '/household_01077010100.csv',
                                   path_us + '/hh_header.csv']
    prefix2csvs[conf.pp_prefix] = [path_us + '/people_01077010100.csv',
